$(function () {
    ...
    function total() {
        $('.godS .ulgoodlist').each(function () {
            ...
            var count = $(this).find('.numT').attr('value')
            price = price.substring(1)
            total = price * count
            $(this).find('.allPriceT').html('￥' + total)
        })
    ...
    function countTotal() {
        ...
        $(".godS .ulgoodlist").each(function () {
            ...
            if (status) {
                var price = $(this).find('.allPriceT').html().substring(1)
                var count = $(this).find('.numT').val()
                sumprice += parseInt(price)
                sumcount += parseInt(count)
            }
        })

        $('.rightTotal i').html(sumcount)
        $('.rightTotal span').html('￥' + sumprice)
